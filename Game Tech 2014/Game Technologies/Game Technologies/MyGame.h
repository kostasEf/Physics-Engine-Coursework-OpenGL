...
protected:
	GameEntity* BuildTreeEntity();

	GameEntity* BuildCubeEntity(float size, Vector3 pos, int angle);

	GameEntity* BuildSphereEntity(float radius, Vector3 pos, Vector3 vel);

	GameEntity* BuildWannaBeSmartEntity(float radius, Vector3 pos, Vector3 vel);

	GameEntity* BuildProjectileEntity(float radius, Vector3 pos, Vector3 vel);

	GameEntity* BuildHurtPigEntity(float radius, Vector3 pos, Vector3 vel);

	GameEntity* BuildQuadEntity(float size);

	GameEntity* ball1;

	GameEntity* wannaBeSmart;

	float time;
	//Speed of the projectile we are shooting
	float projSize;
	//Size of the projectile we are shooting
	float projSpeed;
